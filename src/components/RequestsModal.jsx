...
const RequestsModal = ({ isShowedForm, setIsShowedForm, register, handleSubmit, submit, reset, setIsRequestIdToEdit, isRequestIdToEdit, errors }) => {
    ...
    return (
        <section className={`fixed top-0 left-0 bottom-0 right-0 bg-red flex bg-black/40 justify-center items-center transition-opacity z-20 ${isShowedForm ? "opacity-100 visible" : "opacity-0 invisible"}`}>
            ...
            <form onSubmit={handleSubmit(submit)} className='bg-white p-4 grid gap-4 rounded-md w-[300px] relative overflow-y-auto sm:w-[450px] sm:py-6 sm:px-8'>
                ...
                <h3 className='text-2xl font-bold text-secondary-color'>{isRequestIdToEdit ? "Editar reclamo" : "Nuevo reclamo"}</h3>
                ...
                <button className='bg-primary-color text-black p-2 text-sm border-2 border-transparent hover:text-[0.9rem] hover:border-secondary-color transition-colors rounded-md sm:max-w-max sm:mx-auto sm:px-8'>{isRequestIdToEdit ? "Guardar cambios" : "Añadir reclamo"}</button>
            </form>
